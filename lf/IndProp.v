...
Example Perm3_example2 : Perm3 [1;2;3] [3;2;1].
Proof.
...
Proof.
  ...
  - simpl. apply ev_0.
  - rewrite double_incr. apply ev_SS. apply IH.
Qed.
...
Proof.
  intros n m. intros N M. induction N as [|n' IHN].
  - simpl. apply M.
...
Proof.
  intros n. split.
  - intros EV. induction EV as [| | n m Hn IHn Hm IHm].
  -- apply ev_0.
  -- apply ev_SS. apply ev_0.
  -- apply ev_sum. --- assumption. --- assumption.
  - intros EV. induction EV as [| n' IHn].
  -- apply ev'_0.
  -- apply ev'_sum with (n:= 2) (m := n'). --- apply ev'_2. --- apply IHIHn.
Qed.
...
Proof.
  ...
  induction Hn as [| n' IHn].
    intros m Hnm. simpl in Hnm. apply Hnm.
    intros m Hnm.
    simpl in Hnm. inversion Hnm.
    apply IHIHn. apply H0.
Qed.
...
Proof.
  intros m n o. intros H1 H2. induction H2.
  - apply H1. 
  -rewrite H1. rewrite H2. apply le_S. rewrite H2 in H1. apply le_n.
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | sub_cons1 : forall x l1 l2, subseq l1 l2 -> subseq (x :: l1) (x :: l2)
  | sub_cons2 : forall x l1 l2, subseq l1 l2 -> subseq l1 (x :: l2).

Theorem subseq_refl : forall (l : list nat), subseq l l.
Proof.
  induction l.
  - apply sub_nil.
  - apply sub_cons1. apply IHl.
Qed.
...
Proof.
  ...
  - intros.
    ...
    rewrite Hl1. apply sub_nil.
  - intros. inversion H12 as [| x' l1' l2' H12' | x' l1' l2' H12'].
    + apply sub_nil.
    + apply (sub_cons1 x l1' l3 (IH l1' H12')).
    + apply (sub_cons2 x l1 l3 (IH l1 H12')).
  - intros. apply (sub_cons2 x l1 l3 (IH l1 H12)).
Qed.
...
Proof.
  ...
  - intros H. induction re.
    ...
    + simpl in H. apply orb_true_iff in H. destruct H.
      * apply IHre1 in H. destruct H as [s Hs]. exists s. apply MUnionL. assumption.
      * apply IHre2 in H. destruct H as [s Hs]. exists s. apply MUnionR. assumption.
    + exists []. apply MStar0.
Qed.
...
Proof.
  intros P b H.
  split.
  - intros H0. inversion H.
    + reflexivity.
    + contradiction.
  - intros H0. inversion H.
    + apply H1.
    + unfold not in H1. exfalso. rewrite H0 in H2. inversion H2.
Qed.
...
Proof.
  ...
  - simpl in Hcount.
    destruct (eqbP n m) as [Hnm | Hnm].
    + simpl in Hcount. rewrite Hnm in Hcount.
      simpl in Hcount. inversion Hcount.
    + simpl in Hcount. intros H. destruct H.
      * rewrite H in Hnm. apply Hnm. reflexivity.
      * apply IHl'. apply Hcount. apply H.
Qed.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | ns_nil : nostutter []
  | ns_singleton : forall x, nostutter [x]
...
Proof. 
  repeat constructor; unfold not; intros; discriminate.
Qed.
...
Proof. 
  intro. inversion H. inversion H4. contradiction.
Qed.
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
  | merge_nil : merge [] [] []
  | merge_l : forall x l1 l2 l3, merge l1 l2 l3 -> merge (x :: l1) l2 (x :: l3)
  | merge_r : forall x l1 l2 l3, merge l1 l2 l3 -> merge l1 (x :: l2) (x :: l3).
...
Proof.
  ...
  induction Hmerge.
  - reflexivity.
  - simpl. destruct Hall1 as [Htest1 Hall1'].
    rewrite Htest1. simpl. f_equal. apply IHHmerge; assumption.
  - simpl. destruct Hall2 as [Htest2 Hall2'].
    rewrite Htest2. apply IHHmerge; assumption.
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite rev_app_distr. simpl. rewrite IHpal. rewrite rev_involutive. rewrite <-IHpal. reflexivity.
Qed.
...
Proof.
  ...
  - intros. destruct (EM (In x l1')) as [HIn | HnIn].
    ...
    + apply repeats2.
      ...
      * apply f_equal with (f:=length) in H3. rewrite app_length in H3. rewrite add_comm in H3. simpl in H3.
        rewrite app_length. rewrite add_comm. rewrite H3 in H0. simpl in H0. apply Sn_le_Sm__n_le_m in H0. apply H0.
Qed.
(* ================================================================= *)
...
Proof.
  ...
  - intros.
    ...
    + rewrite H1. assert (eq: a :: s1 ++ s2 = (a :: s1) ++ s2). reflexivity. rewrite eq. apply MApp. apply H2. apply H3.
Qed.
