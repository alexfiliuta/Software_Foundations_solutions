...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros. injection H as H1 H2. rewrite <- H2 in H0. injection H0 as H3. rewrite H3. rewrite H1. reflexivity. Qed.
...
Proof.
  ...
  - destruct m as [| m'].
    -- simpl. reflexivity.
    -- simpl. intro eq. discriminate eq.
  - intros m eq. destruct m as [| m'] eqn:E.
    -- simpl in eq. discriminate eq.
    -- f_equal. apply IHn'. simpl in eq. rewrite eq. reflexivity. Qed.
...
Proof.
  ...
  - simpl. intros m eq. destruct m as [| m'].
    -- reflexivity.
    -- simpl in eq. symmetry in eq. discriminate eq.
  - simpl. destruct m as [| m'] eqn:E.
    -- simpl. intros eqFalse. discriminate eqFalse.
    -- intro suppose.
       simpl in suppose.
       f_equal.
       injection suppose as goal. rewrite <- plus_n_Sm in goal. rewrite <- plus_n_Sm in goal.
       injection goal as goal2. apply IHn'. apply goal2.
Qed.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
  ...
  nth_error l n = None.
Proof.
  intros. generalize dependent n. induction l as [| x xs IHl'].
  - intros n H. simpl in H. rewrite <- H. simpl. reflexivity.
...
Proof.
  ...
  - intros l1 l2 H. simpl in H. injection H as goal1 goal2. rewrite <- goal1. rewrite <- goal2. simpl. reflexivity.
  - intros l1 l2 H. simpl in H. destruct (split t) as [lx ly]. injection H as goal1 goal2. rewrite <- goal1.
  rewrite <- goal2. simpl. f_equal. apply IHt. reflexivity.
Qed.
...
Proof.
  intros. destruct b.
  - destruct (f true) eqn:IHtrue.
    + rewrite IHtrue. rewrite IHtrue. reflexivity.
    + destruct (f false) eqn:IHfalse.
      * rewrite IHtrue. reflexivity.
      * rewrite IHfalse. reflexivity.
  - destruct (f false) eqn: IHfalse.
    + destruct (f true) eqn: IHtrue.
      * rewrite IHtrue. reflexivity.
      * rewrite IHfalse. reflexivity.
    + rewrite IHfalse. rewrite IHfalse. reflexivity.
Qed. 
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros. apply eqb_true in H. apply eqb_true in H0. assert (Heq: n = p).
  {transitivity m. assumption. assumption. } - rewrite Heq. apply eqb_refl.
Qed.
...
Proof.
  ...
  intros X Y l lx ly Hlen.
  generalize dependent ly.
  induction lx as [|x xs IHx].
  intros [|y ly'].
  - reflexivity.
  - intros Hlen. simpl in Hlen. discriminate Hlen.
  -intros [|y ly'].
    + intros Hlen. simpl in Hlen. discriminate Hlen.
    + intros Hlen. simpl in Hlen. injection Hlen as goal. specialize (IHx ly' goal). simpl. rewrite IHx. reflexivity.
Qed.
(* Do not modify the following line: *)
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X f x l lf. generalize dependent x. generalize dependent lf.
  induction l as [| a l' IHl'].
  - simpl. intros lf x check. discriminate check.
  - intros lf x. simpl. destruct (f a) eqn:Ha.
    + intros H. injection H as Hhead Htail. destruct lf as [| y lf'].
      -- rewrite <- Hhead. assumption.
      -- rewrite <- Hhead. assumption.
    + apply IHl'.
Qed.
...
Proof. intros X test l. induction l as [| x xs IHl].
  - simpl. reflexivity.
  - simpl. destruct (test x) eqn:Tx.
    + simpl. unfold existsb'. simpl. rewrite Tx. simpl. reflexivity.
    + simpl. unfold existsb'. simpl. rewrite Tx. simpl. rewrite IHl. unfold existsb'. reflexivity.
Qed.
