...
Proof.
  intros P Q NPQ.
  split. unfold not in NPQ.
  - unfold not. intro HP. apply NPQ. left. apply HP.
  - unfold not. intro HQ. apply NPQ. right. apply HQ.
Qed.
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R. intros IH1. intros IH2. split.
  - intros IHP. apply IH2. apply IH1. apply IHP.
  - intros IHR. apply IH1. apply IH2. apply IHR.
Qed.
...
Proof.
  intros P Q R. split.
  - intros [IH1 | IH2]. split.
    + left. apply IH1.
    + left. apply IH1.
    + split. ++ right. apply IH2. ++ right. apply IH2.
  - intros [[HP | HQ] [HP' | HR]].
    + left. apply HP.
    + left. apply HP.
    + left. apply HP'.
...
Proof.
  intros X P x. unfold not. intros [ih IH]. apply IH.  apply x.
Qed.
...
Proof.
  intros X P Q. split.
  - intros [e [IHp | IHq]]. 
    + left. exists e. apply IHp.
    + right. exists e. apply IHq.
  - intros [[e IHp] | [e IHq]].
    + exists e. left. apply IHp.
    + exists e. right. apply IHq.
Qed.
...
Proof.
  intros n m. intros H. revert n H. induction m as [| m' IHm].
  - intros n H. destruct n.
    + exists 0. reflexivity.
    + simpl in H. discriminate H.
  - intros [|n] H.
    + exists (S m'). simpl. reflexivity.
    + apply IHm in H. destruct H as [e He]. rewrite He. exists e. simpl. reflexivity.
Qed.

Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof.
  intros n m H. revert n H. induction m as [|m' IHm].
  - intros n H. destruct n.
    + simpl. reflexivity.
    + simpl in H. destruct H as [e He]. discriminate He.
  - intros [|n'] H.
    + simpl. reflexivity.
    + apply IHm. destruct H as [e He]. exists e. injection He as goal. apply goal.
Qed.
...
Proof.
  ...
  { induction l as [|x l' IHl'].
  - simpl. contradiction.
  - simpl. intro IH2. destruct IH2 as [e | e].
    + exists x. split. -- apply e. -- left. reflexivity.
    + apply IHl' in e. destruct e as [x0 [S1 S2]]. exists x0. split. -- apply S1. -- right. apply S2. }
  { induction l as [|x l' IHl].
  - simpl. intros [e [I1 I2]]. apply I2.
  - intros [x' [HX1 HX2]]. simpl in HX2. simpl. destruct HX2 as [Scope1 | Scope2].
    + left. rewrite <- Scope1 in HX1. apply HX1.
    + right. apply IHl. exists x'. split. assumption. assumption. }
Qed.
...
Proof.
  ...
  - split.
    ...
    -- simpl. intros [x1|x2]. --- contradiction. --- apply x2.
  - split.
    -- simpl. intros H. destruct H as [x1|x2]. ---left. left. apply x1. --- apply IH in x2. destruct x2 as [xx1|xx2].
  ---- left. right. apply xx1.
  ---- right. apply xx2.
    -- simpl. intros [x1|x2]. { destruct x1 as [y1 | y2]. - left. apply y1. - right. apply IH. left. apply y2. }
                              { right. apply IH. right. apply x2. }
Qed.
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P l. induction l as [|lhead ltail IHl].
  { simpl. split. - intros H. apply I. - intros H f. contradiction. }
  { simpl. split. - intros H. split. -- apply H. left. reflexivity. -- apply IHl. intros x IHn. apply H. right. apply IHn.
  - intros [I1 I2]. intros x Q. destruct Q as [Q1 | Q2]. -- rewrite <- Q1. apply I1. -- assert (HF: forall x : T, In x ltail -> P x). {apply IHl. apply I2. }
  apply HF. apply Q2. }
Qed.
...
Proof.
  intros Podd Peven n Hodd Heven. unfold combine_odd_even. destruct (even n) eqn:Truth.
  - apply Heven. unfold odd. rewrite Truth. simpl. reflexivity.
  - apply Hodd. unfold odd. rewrite Truth. simpl. reflexivity.
Qed.
...
Proof.
  ...
  unfold combine_odd_even in H. destruct (even n) eqn:E.
  - unfold odd in Hodd. rewrite E in Hodd. simpl in Hodd. discriminate Hodd.
  - apply H.
Qed.
...
Proof.
  ...
  unfold combine_odd_even in H. destruct (even n) eqn:E.
  - apply H.
  - unfold odd in Heven. rewrite E in Heven. simpl in Heven. discriminate Heven.
Qed.
...
Proof.
  ...
  - rewrite even_S. destruct (even n') eqn:E. -- simpl. destruct IHn as [k H]. exists k. rewrite <- H. reflexivity.
  -- simpl. destruct IHn as [k H]. exists (S k). rewrite H. simpl. reflexivity.
Qed.
...
Proof.
intros b1 b2. split.
  - intro H. split. -- destruct b1 eqn:E. --- reflexivity. --- simpl in H. apply H. -- destruct b2 eqn:E. --- reflexivity.
    --- rewrite andb_false_r in H. apply H.
...
Proof.
intros b1 b2. split.
  - intros H. destruct b1 eqn:E. -- left. reflexivity. -- simpl in H. right. apply H.
  - intros H. destruct b1 eqn:E. -- simpl. reflexivity. -- simpl. destruct H as [H1 | H2]. --- discriminate H1. --- apply H2.
Qed.
...
Proof.
  intros x y. split.
  - intros Hneq. unfold not. intros Heq. rewrite Heq in Hneq. rewrite eqb_refl in Hneq. discriminate Hneq.
  - intros Hneq. unfold not in Hneq. destruct (x =? y) eqn:Heq. -- apply eqb_eq in Heq. apply Hneq in Heq. contradiction Heq.
  -- reflexivity.
Qed.
...
Proof.
  intros A eqb Heqb. intros l1 l2.
  split.
  -
    revert l2. induction l1 as [|l1head l1tail IH]; intros [|l2head l2tail]; simpl; try intro H; try discriminate H.
    + reflexivity.
    + apply andb_true_iff in H as [Heq Htail].
      f_equal.
      * apply Heqb; assumption.
      * apply IH; assumption.
  - 
    revert l2. induction l1 as [|l1head l1tail IH]; intros [|l2head l2tail]; simpl; try intros H; inversion H; subst.
    + reflexivity.
...
Proof.
  ...
  - induction l as [| x xs IHl].
    + simpl. reflexivity.
    + simpl. intros Hyp. apply andb_true_iff in Hyp. destruct Hyp as [Hx Hxs]. split.
      * assumption.
      * apply IHl in Hxs. assumption.
  - induction l as [| x xs IHl].
    + simpl. reflexivity.
    + simpl. intros [Hx Hxs]. rewrite Hx. apply IHl in Hxs. rewrite Hxs. reflexivity.
Qed.
...
Proof.
  ...
  induction l as [| x l' IHl].
  - reflexivity.
  - simpl. rewrite <- IHl. unfold tr_rev. simpl. rewrite rev_append_distr_app. reflexivity.
Qed.
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  intros P H. apply H. right. intros NP. apply H. left. apply NP. 
Qed.
...
Proof.
  ...
  destruct (EM (P x)) as [Hpx | Hnpx].
    - exact Hpx.
    - exfalso. apply H. exists x. exact Hnpx.
Qed.
...
Lemma EM_to_double_negation_elimination : excluded_middle -> double_negation_elimination.
Proof.
  intros EM P HNNP.
...
Lemma DNE_to_DeMorgan : double_negation_elimination -> de_morgan_not_and_not.
Proof.
  intros DNE P Q H.
  apply DNE.
  intros HN.
  apply H.
  split.
  - intros HP. apply HN. left. exact HP.
  - intros HQ. apply HN. right. exact HQ.
Qed.

Lemma DeMorgan_to_ImpliesToOr : de_morgan_not_and_not -> implies_to_or.
Proof.
  intros DEM. intros P Q IMP. apply DEM. unfold not. intro PROP.  destruct PROP as [C1 C2]. apply C1. intros IHP.
apply C2. apply IMP. assumption.
Qed.

Lemma ImpliesToOr_to_CM : implies_to_or -> consequentia_mirabilis.
Proof.
  intros ImpliesOr P H.
  destruct (ImpliesOr P P (fun x => x)) as [HNP | HP].
  - apply H. exact HNP.
  - exact HP.
Qed.

Lemma CM_to_Peirce : consequentia_mirabilis -> peirce.
Proof.
  intros CM P Q PIERCE. apply CM. intros HYP. apply PIERCE. intros HYP2. exfalso. apply HYP. apply HYP2.
Qed.

Lemma Peirce_to_EM : peirce -> excluded_middle.
Proof.
  intros PEIRCE. intros P. apply PEIRCE with (Q := False). intros H. right. intros HP. apply H. left. apply HP.
Qed.
