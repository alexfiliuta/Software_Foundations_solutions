...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Theorem: Addition is commutative. For any n and m natural, n+m = m+n
    Proof: By induction on n:
        First suppose n is 0 : we have to prove 0 + m = m + 0. This follows from +.
        Next, suppose n is S n', where n' + m = m + n'. We have to prove that S n' + m = m + S n'. And by def of + we get:
          S (n' + m) = S (m + n'). which can be rewritten using the IH as S (m + n') = S (m + n'). By virtue of reflexivity this is true. Qed.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    Proof:
    Base case: Consider \(n = 0\). According to the definition of the equality check,
    \(0 =? 0\) evaluates directly to true by the reflexivity of equality.
    Thus, the statement holds for \(n = 0\).
    Inductive step: Assume the statement is true for some arbitrary natural number \(n\);
    that is, assume \(n =? n = \text{true}\). We need to show that the statement holds for
    \(n + 1\), i.e., \(S(n) =? S(n) = \text{true}\).
    By the definition of the equality check on natural numbers, \(S(n) =? S(n)\) simplifies
    to \(n =? n\). By our inductive hypothesis, we know that \(n =? n\) evaluates to true.
    Hence, \(S(n) =? S(n)\) also evaluates to true.
    Conclusion: By the principle of mathematical induction, we have shown that
    \(n =? n = \text{true}\) for any natural number \(n\).
*)
...
Lemma mul_succ_l : forall n k : nat,
  n * S k = n + n * k.
Proof.
  intros n k.
  induction n as [| n' IHn']; simpl.
  - reflexivity.
  - rewrite IHn'.  (* Use the induction hypothesis to rewrite S n' * S k = S n' + n' * S k *)
    rewrite add_comm.  (* Reorder terms in one of the additions to align them *)
    rewrite <- add_assoc.  (* Align for applying plus_n_Sm *)
    replace (n' * k + k) with (k + n' *k).
    -- reflexivity.
    -- rewrite add_comm. reflexivity.
Qed.
...
Proof.
  ...
  - induction n as [| n' IHn']; simpl.
    + reflexivity.
    + assumption.
  - rewrite mul_succ_l. rewrite IHm'.
    induction n as [| n' IHn']; simpl.
    + reflexivity.
    + reflexivity.
Qed.
...
Proof. intros. simpl. rewrite add_0_r.  reflexivity.
Qed.
...
Theorem add_shuffle3' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof. intros. rewrite add_assoc. rewrite add_assoc. replace (n + m) with (m + n).
  - reflexivity.
  - rewrite add_comm. reflexivity.
Qed.
...
Proof.
  intros. destruct b as [|b0|b1] eqn:E.
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
...
(*The conversion from binary to natural numbers and back can lose information
 about leading zeros in binary representations. For example, both B0 Z
 (0 in binary with a leading zero) and Z (just 0) convert to the same natural number 0.
 Converting 0 back to binary results in Z, losing any information about the original leading zeros.
 This discrepancy results from multiple binary representations (like B0 Z, B0 (B0 Z), etc.)
 all simplifying to the same natural number. To address this, normalizing binary numbers before
 conversion ensures each natural number maps back to a unique binary representation,
 eliminating ambiguities caused by leading zeros. *)
...
Lemma incr_double_bin : forall b, incr (double_bin (double_bin b)) = B1 (double_bin b).
Proof.
  intros b.
  simpl.
  induction b as [| b' | b'].
...
Proof.
  ...
  - simpl. rewrite <- IHb'.
    rewrite <- double_plus. rewrite -> nat_to_bin_double.
    rewrite IHb'. destruct b' as [|b''|b''].
      -- simpl. reflexivity.
      -- simpl. assert (H: incr (double_bin (double_bin (normalize b''))) = B1 (double_bin (normalize b''))).
