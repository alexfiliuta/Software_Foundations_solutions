...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | x :: t => if even x then oddmembers t else x :: oddmembers t
  end. 
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  ...
  | nil , _ => l2
  | _ , nil => l1
  | h1 :: r1 , h2 :: r2 => h1 :: h2 :: alternate r1 r2
  end. 
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => O
  | h :: y => if h =? v then S (count v y) else (count v y)
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h::t => if (count h s2) <? (count h t) + 1 then false else included t s2
  end.
...
Proof. intros. induction l1 as [| n l' IHl'].
...
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof. intros. induction l as [| n l' IHl'].
  - simpl. reflexivity.
...
Theorem eqblist_refl : forall l:natlist,
  true = eqblist l l.
Proof.
  intros. simpl. induction l as [| n l' IHl'].
  - simpl. reflexivity.
...
Proof.
  intros l1 l2 H.
  rewrite <- rev_involutive with (l := l1). rewrite <- rev_involutive with (l := l2).
...
Proof.
  intros. destruct x as [n]. simpl. rewrite eqb_refl. reflexivity.
Qed. 
...
Proof.
  intros. simpl. replace (eqb_id x x) with true.
  - reflexivity.
