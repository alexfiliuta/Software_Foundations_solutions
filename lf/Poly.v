...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros. induction l as [|x xs IHx'].
  - simpl. reflexivity.
...
Proof.
  intros. induction l1 as [| x xs IHl'].
  -simpl. reflexivity.
  - simpl. rewrite IHl'. reflexivity.
Qed.
...
Proof.
  intros. induction l as [| h l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. assert (map f (rev l' ++ [h]) = map f (rev l') ++ map f [h]).
    { rewrite map_plus. reflexivity. } rewrite H. simpl. reflexivity.
Qed.
...
Proof.
  intros. induction l as [| x xs IHl'].
  - reflexivity.
  - simpl. replace (fold_length (x :: xs)) with (S (fold_length xs)).
...
Example test_fold_map1 :
  fold_map (fun x => x + 1) [1; 2; 3] = [2; 3; 4].
Proof. simpl. reflexivity. Qed.
...
Theorem fold_map_first_correct: forall X Y (f: X -> Y) (x: X) (xs : list X),
  fold_map f (x::xs) = f x :: fold_map f xs.
Proof.
  intros. induction xs as [| y ys IH].
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
...
Proof.
  intros. induction l as [| x xs IHl'].
  - simpl. reflexivity.
  - simpl. rewrite fold_map_first_correct. rewrite IHl'. reflexivity.
Qed.
